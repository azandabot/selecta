...
if selecta_ipc_up; then
	...
	# Clearing must be final. Rendering the bar and erasing it within the same
	# tick left it non-empty for part of every tick, and the launcher reads on
	# its own schedule, so the bar blinked.
	_flicker=0
	_fi=0
	while [ "$_fi" -lt 8 ]; do
